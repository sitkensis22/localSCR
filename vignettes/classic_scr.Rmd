...
N = 200, K = 4, base_encounter = 0.10, enc_dist = "binomial", hab_mask = FALSE, setSeed = 100)
...
N = 200, K = 4, base_encounter = 0.15, enc_dist = "binomial",hab_mask = hab_mask, setSeed = 100)
...
s.stdata = as.data.frame(cbind(s.st3d,Det=c(as.numeric(apply(data3d$y,1,sum)>0),rep(0,M-nrow(data3d$y)))))
...
ggplot() + geom_point(data=as.data.frame(Grid$grid),aes(x=x,y=y),color="grey60",size=1.25) + 
    ...
    theme_classic() + ylab("Northing") + xlab("Easting") +
    scale_x_continuous(expand=c(0.025, 0.025)) + scale_color_manual(values = c("orangered","springgreen")) +
    scale_y_continuous(expand=c(0.025, 0.025)) + 
    theme(axis.text = element_text(size=12),axis.title = element_text(size=16))
```
